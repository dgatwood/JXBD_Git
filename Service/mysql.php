<? set_time_limit(0);if($_GET['s']) {highlight_file(__FILE__);die();} ?>
<html>
<head>
...
<? foreach(Array("","SHOW TABLES","SHOW DATABASES","SELECT * FROM ``","SHOW CREATE TABLE ``","EXPLAIN ``","CHECK TABLE ``","OPTIMIZE TABLE ``","REPAIR TABLE ``","BACKUP TABLES") as $v) echo "<option value='$v'>$v</option>"; ?>
</select><br />
<textarea name='query' rows='10' cols='50'><?=stripslashes($_POST['query']);?></textarea><br /><input type='submit' name='submit' value='Run Query' /></form>
<?
if($_POST['tables']){
